...
Search::direct_search_matching(IndexValueItem &value, MatchOrder *tc_order, OfflineIndex *tc_offline_index, Arg *tc_arg,
    ...
    if(tc_arg->is_post_verify_time_constrain == "n"){
    ...
    } else if(tc_arg->is_post_verify_time_constrain == "y"){
...
void Search::recursive_search(IndexValueItem &value,
    ...
    if(value.self_key == _search_order.front()){ // If it is the starting point.
        ...
        if(tc_arg->is_post_verify_time_constrain == "n"){
            ...
            for(auto &one_result_it:vec_one_result){
                this->only_struct_match_count += 1;
                if(this->is_satisfy_time_constrain(one_result_it, tc_offline_index)){
                    this->match_count += 1;
                }
            }
...
bool Search::is_satisfy_time_constrain(std::unordered_map<IndexKeyItem, IndexValueItem, IndexKeyItemHash> &one_result,
                                       OfflineIndex *tc_offline_index) {
    for(auto &it:one_result){
        auto &key = it.first;
        auto &value = it.second;

        auto &prec_key_vec = tc_offline_index->q_edge_to_all_prec_time_order.at(key);

        if(prec_key_vec.empty()){  // if 'key' no prec keys
            return true;
        } else{  // if 'key' has prec keys
            bool is_satisfy_all_prec_key = true;

            for(auto &prec_key:prec_key_vec){
                if(one_result.at(prec_key).timestamp > value.timestamp){  // if not satisfy time constrain
                    is_satisfy_all_prec_key = false;
                    break;
                }
            }

            if(is_satisfy_all_prec_key){   // if satisfy all time constrain
                return true;
            } else{
                return false;
            }
