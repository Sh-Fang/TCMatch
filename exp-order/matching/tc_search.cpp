...
Search::direct_search_matching(IndexValueItem &value, MatchOrder *tc_order, OfflineIndex *tc_offline_index, Arg *tc_arg,
    ...
    if(tc_arg->matching_order_type == "GQL"){
        this->dynamic_generate_matching_order_GQL(value.self_key, _search_order, tc_global_index->global_index);
    }else if(tc_arg->matching_order_type == "NLF"){
        this->dynamic_generate_matching_order_NLF(value.self_key, _search_order, tc_global_index->global_index);
    }else{
        _search_order = tc_order->search_order.at(value.self_key);
    }
...
IndexKeyItem Search::dynamic_get_next_order_NLF(IndexKeyItem &key,
                                            std::vector<IndexKeyItem> &_search_order,
                                            std::unordered_map<IndexKeyItem, std::vector<IndexValueItem>, IndexKeyItemHash> &global_index) {
    IndexKeyItem min_key = key;
    uit min_candidate = 99999;

    for(auto &it:global_index.at(key)){
        for(auto &[nei_key,nei_pos_vec]:it.neighbor_link){

            if(std::find(_search_order.begin(), _search_order.end(),nei_key) == _search_order.end()){
                int status_true_candidate_num = 0;
                for(auto &nei_pos:nei_pos_vec){
                    auto &nei_value = global_index.at(nei_key).at(nei_pos);
                    if(nei_value.status){
                        status_true_candidate_num += 1;
                    }
                }
                if(status_true_candidate_num < min_candidate){
                    min_candidate = status_true_candidate_num;
                    min_key = nei_key;
                }
            }
...
IndexKeyItem Search::dynamic_get_next_order(IndexKeyItem &key,
                                            std::vector<IndexKeyItem> &_search_order,
                                             std::unordered_map<IndexKeyItem, std::vector<IndexValueItem>, IndexKeyItemHash> &global_index) {
    IndexKeyItem min_key = key;
    uit min_candidate = 99999;
    ...
    for(auto &it:global_index.at(key)){
        for(auto &[nei_key,nei_pos_vec]:it.neighbor_link){
            ...
            if(std::find(_search_order.begin(), _search_order.end(),nei_key) == _search_order.end()){
                if(nei_pos_vec.size() < min_candidate){
                    min_candidate = nei_pos_vec.size();
                    min_key = nei_key;
                }
            }
...
void Search::dynamic_generate_matching_order_NLF(IndexKeyItem &key,
                                                 std::vector<IndexKeyItem> &_search_order,
                                                 std::unordered_map<IndexKeyItem, std::vector<IndexValueItem>, IndexKeyItemHash> &global_index) {

    if(_search_order.size() == global_index.size()){
        return;
    }else{
        _search_order.emplace_back(key);

        IndexKeyItem next_order = dynamic_get_next_order_NLF(key,_search_order,global_index);

        if(next_order != key){
            dynamic_generate_matching_order_NLF(next_order, _search_order, global_index);
        }
    }
}


void Search::dynamic_generate_matching_order_GQL(IndexKeyItem &key,
                                                 std::vector<IndexKeyItem> &_search_order,
                                                 std::unordered_map<IndexKeyItem, std::vector<IndexValueItem>, IndexKeyItemHash> &global_index) {
    ...
    if(_search_order.size() == global_index.size()){
        ...
        IndexKeyItem next_order = dynamic_get_next_order(key,_search_order,global_index);

        if(next_order != key){
            dynamic_generate_matching_order_GQL(next_order, _search_order, global_index);
        }
    }
}
