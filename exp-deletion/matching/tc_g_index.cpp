...
void GIndex::delete_g_edge(IndexValueItem &value) {
    auto &source_id = value.v_source_id;
    auto &target_id = value.v_target_id;
    ...
    g_index[source_id][us_ut_el_sl_tl]["s"].erase(g_index[source_id][us_ut_el_sl_tl]["s"].begin() + pos);
    g_index[target_id][us_ut_el_sl_tl]["t"].erase(g_index[target_id][us_ut_el_sl_tl]["t"].begin() + pos);
}
