...
void IndexValueItem::clear() {
    ...
    self_key.clear();
    neighbor_link.clear();
    duplicated_edge_timestamp.clear();
}
...
void GlobalIndex::delete_fht(IndexValueItem &value) {
    std::string edge_to_str = std::to_string(value.v_source_id) + "_" + std::to_string(value.v_target_id) + "_" + std::to_string(value.edge_label) + "_" + std::to_string(value.v_source_label) + "_" + std::to_string(value.v_target_label);

    auto map_finder = this->fht[value.self_key].find(edge_to_str);

    if (map_finder != this->fht[value.self_key].end()) {
        this->fht[value.self_key].erase(edge_to_str);
    }
}
...
//        Label label{edge_.v_source_label, edge_.v_target_label, edge_.edge_label};
        ...
        auto finder = tc_offline_index->label_to_all_q_edge.find({edge_.v_source_label, edge_.v_target_label, edge_.edge_label});
...
bool GlobalIndex::deletion_process(OfflineIndex *tc_offline_index, Search *tc_search, MatchOrder *tc_order, Arg *tc_arg,
                                   GIndex *tc_g_index, IO *tc_io) {

    for(auto &edge_:tc_io->S_delete){

        // if this edge already existed in global-index, then find it by g-index and get matches
        // when deletion process, delete this edge itself and all it's succ-edges, and update all related super-node's status.
        // if this edge is duplicated edge, then using 'search_matching_with_specified_timestamp' to get matches and deleting duplicated timestamp from super-node.
        // if deletion happen, both fht and g-index all need to be deleted.
        ...
        if(finder != tc_offline_index->label_to_all_q_edge.end()){
            ...
            for(auto &key:finder->second){
                ...
                if(this->fht.at(key).find(edge_to_str) != this->fht.at(key).end()){
                    ...
                    if(existed_value.timestamp != edge_.timestamp){

                        auto duplicated_time_finder = std::find(existed_value.duplicated_edge_timestamp.begin(),existed_value.duplicated_edge_timestamp.end(),edge_.timestamp);

                        if(duplicated_time_finder != existed_value.duplicated_edge_timestamp.end()){
                            this->search_time_start = T_NOW;

                            tc_search->direct_search_matching_with_specified_timestamp(existed_value,edge_.timestamp,tc_order,tc_offline_index,tc_arg,this);

                            existed_value.duplicated_edge_timestamp.erase(duplicated_time_finder);
                        ...
                        tc_search->direct_search_matching(existed_value,tc_order,tc_offline_index,tc_arg, this);


                        this->delete_double_link(existed_value);
                        this->delete_fht(existed_value);
...
void GlobalIndex::delete_double_link(IndexValueItem &value) {

    // delete all neighbors to self link.
    for(auto &map_it:value.neighbor_link){
        auto &nei_key = map_it.first;
        auto &nei_pos_vec = map_it.second;
        for(auto &nei_pos:nei_pos_vec){
            auto &nei_value = this->global_index.at(nei_key).at(nei_pos);
            auto self_pos_finder = std::find(nei_value.neighbor_link.at(value.self_key).begin(),nei_value.neighbor_link.at(value.self_key).end(),value.self_pos);
            if(self_pos_finder != nei_value.neighbor_link.at(value.self_key).end()){
                nei_value.neighbor_link.at(value.self_key).erase(self_pos_finder);
            }
            nei_value.update_self_status();
        }
    }
    ...
    // delete self super-node.
    value.clear();
//    this->global_index.at(value.self_key).erase(this->global_index.at(value.self_key).begin() + value.self_pos);


}

void IndexKeyItem::clear() {
    qe.u_source_id    = 0;
    qe.u_target_id    = 0;
    qe.u_edge_label   = 0;
    qe.u_source_label = 0;
